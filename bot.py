...
# When current price is BELOW the EMA, the resistance zone we short into must
...
def check_and_trade(symbol, row, df, all_state):
    ...
    #         OR price within PATH_C_BELOW_EMA_PROXIMITY_PCT below EMA
    #         OR price ≥ PATH_C_ABOVE_EMA_EXTENDED_PCT above EMA
    #     - Find nearest confluence resistance zone ABOVE current price
    #     - FAKEOUT FILTER: if price is currently below EMA, the zone_high
    #       must sit at least PATH_C_BELOW_EMA_RESISTANCE_GAP_PCT below the EMA
    #       (zones straddling/touching the EMA are rejected as fakeouts)
        ...
        if path_c_arm_ok:
            ...
            if zone is not None:
                if zone["low"] > last_close:
                    # ── FAKEOUT FILTER: when price is below EMA, require the
                    # entire zone to sit at least PATH_C_BELOW_EMA_RESISTANCE_GAP_PCT
                    # below the EMA. This stops us from arming on zones that
                    # straddle / cling to the EMA, which produce frequent fakes.
                    if last_close < ema_now:
                        max_zone_high_allowed = ema_now * (1 - PATH_C_BELOW_EMA_RESISTANCE_GAP_PCT / 100.0)
                        if zone["high"] > max_zone_high_allowed:
                            print(f"[PATH-C] {symbol} — REJECT zone (fakeout filter): "
                                  f"price below EMA, zone_high {round(zone['high'], precision)} > "
                                  f"EMA−{PATH_C_BELOW_EMA_RESISTANCE_GAP_PCT}% threshold "
                                  f"{round(max_zone_high_allowed, precision)} (EMA {round(ema_now, precision)})")
                            save_state(all_state)
                            return
                    ...
                    print(f"[PATH-C] {symbol} — ARMING zone {round(zone_low, precision)}–{round(zone_high, precision)} "
                          ...
                          f"trigger: {trigger_str}, ema_diff {round(ema_diff_pct, 2)}%)")
...
send_telegram(
    ...
    f"🆎 Path C    : <code>(pump≥{PATH_C_MIN_PUMP_PCT}% OR below EMA within {PATH_C_BELOW_EMA_PROXIMITY_PCT}% OR above EMA ≥{PATH_C_ABOVE_EMA_EXTENDED_PCT}%) + nearest multi-TF pivot zone above price ({MIN_TF_CONFLUENCE}/{len(PATH_C_ENABLED_TIMEFRAMES)} TFs); when price&lt;EMA, zone_high must be ≤ EMA−{PATH_C_BELOW_EMA_RESISTANCE_GAP_PCT}% (fakeout filter) → 30m break-below (≤{PATH_C_MAX_WAIT_BARS} bars)</code>\n"
